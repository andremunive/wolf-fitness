...
import { BeneficiaryType, CreateLoanInput } from '../../models/prestamo.model';
...
@Component({
  ...
  styleUrls: ['./new-loan-modal.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class NewLoanModalComponent implements OnInit, OnDestroy {
  ...
  // Today as YYYY-MM-DD, used as max for the date input
  ...
  private buildForm(): void {
    this.form = this.fb.group({
      beneficiary_type: ['', Validators.required],
      trainer_id:       [null],
      provider_id:      [null],
      third_party_name: [''],
      amount_cop:       [null, [Validators.required, Validators.min(1)]],
      loan_date:        [this.today, Validators.required],
      notes:            ['']
    });
  }
  ...
  selectBeneficiaryType(type: BeneficiaryType): void {
    ...
    // Reset beneficiary-specific fields and selections
  ...
  onTrainerBlur(): void {
    setTimeout(() => {
      this.isTrainerDropdownOpen = false;
      // Restore display to selected trainer name or clear
  ...
  save(): void {
    ...
    this.prestamosService.createLoan(payload)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
          this.isSaving = false;
        ...
        error: (err: unknown) => {
          this.isSaving = false;
          this.errorMessage = err instanceof Error ? err.message : 'Error al registrar el préstamo.';
          this.toast.error(this.errorMessage ?? 'Error al registrar el préstamo.');
          this.cdr.markForCheck();
        }
      });
  }
